...
class ValgrindTest(Test):
    ...
    def __init__(self, cwd, type, path, filename, arguments, format):
        super(ValgrindTest,self).__init__(cwd, type, path, filename, arguments, format)

    def report_identifiers(self):
        return ['Valgrind', 'memory', 'test', os.path.join(self.cwd, self.filename)]
